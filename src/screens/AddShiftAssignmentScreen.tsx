...
export function AddShiftAssignmentScreen({ navigation }: Props) {
  ...
  const [modalType, setModalType] = useState<"employee" | "shift" | "line">(
    "employee",
  );
  ...
  const validateAssignment = (): string | null => {
    ...
    if (days > maxRangeDays) {
      return "Assignment range is too long (max 1 year)";
    }
  ...
  const handleAssign = async () => {
    ...
    if (!employee || !shift) {
      Alert.alert("Validation Error", "Please select an employee and a shift");
      return;
    }
    ...
    try {
      await shiftApi.assignShift({
        ...
        employeeName: employee.fullName, // Not used by Create but needed for ShiftAssignment type in FE
        shiftName: shift.name, // Not used by Create but needed for ShiftAssignment type in FE
      });
      ...
      const errorMsg =
        ...
        "An error occurred while assigning shift";
          ...
          >
            <View style={styles.pickerContent}>
              ...
              <Text
                style={[
                  styles.pickerText,
                  !selectedLine && styles.placeholderText,
                ]}
              >
